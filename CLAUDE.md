...
`cheatsheet-body.html` and `poster-body.html` in German, `cheatsheet-body.en.html` and
`poster-body.en.html` in English. They are the only source files. `.github/workflows/deploy.yml` wraps each into a full
...
Each `.en.html` is derived from its German counterpart by replacing prose only: the markup,
the SVG geometry and every class name stay identical, so the pair diffs cleanly. Keep it
that way — a structural change belongs in both.

The sheet's translation was generated by walking its text nodes and replacing them by
position, which is why the pair has to stay structurally identical. Three things are not
text nodes and were translated separately: the `.tpl` blocks, whose line breaks are load-
bearing; the inline script's `FINDER` and `PNOTES` strings; and the `aria-label` attributes.
Anchor ids stay German (`#ebenen`, `#artefakte`) in both languages — they are identifiers,
not prose, and keeping them identical keeps the inline script identical too.
